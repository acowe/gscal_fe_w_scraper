...
function NavHead(props){
    const fName = props.uName.split(" ")[0];
    //href={"/gscal_front_end/#/sign_out"}
    ...
    return(
        <Row className={"py-4 header"}>
            ...
            <Col xs={10} sm={8} md={6} lg={8} className={"d-flex justify-content-sm-center align-middle ps-sm-0"}>
                <h5 className={"my-0 fs-4 fw-bold h_title"}>
                    gradescope calendar</h5>
            </Col>
            <Col xs={2} sm={2} md={3} lg={2} className={"h_other"}>
                <a className={"w-75 py-0 btn btn-light btn-block shadow-none border-0 rounded-pill h_other_text"}
                ...
                <Dropdown>
                    ...
                    <Dropdown.Menu>
                        ...
                        <Dropdown.Divider/>
                        <Dropdown.Item className={"text-danger"} onClick={(e)=> props.logOut()}>Sign out</Dropdown.Item>
